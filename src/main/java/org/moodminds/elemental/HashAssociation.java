 ...
 */
public class HashAssociation<K, V> extends WrapAssociation<K, V, Map<K, V>>
    ...
    public HashAssociation(KeyValue<? extends K, ? extends V>... kvs) {
        this(new HashMap<>(max((int) (kvs.length/.75f) + 1, 16)), Stream.of(kvs));
    }
    ...
    public HashAssociation(Container<? extends KeyValue<? extends K, ? extends V>> kvs) {
        this(new HashMap<>(max((int) (kvs.size()/.75f) + 1, 16)), kvs.stream());
    }
