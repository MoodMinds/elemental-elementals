...
public abstract class AbstractMapSet<E, M extends Map<E, E>>
     ...
     */
    public AbstractMapSet(M map, Producer<? extends E> elements) {
    ...
    protected Iterator<E> iterator(E value, boolean present, Runnable removal) {
        return new Iterator<E>() {
            ...
            @Override public E next() { modCheckIterator.next(); return iterator.next(); }
            @Override public void remove() { iterator.remove(); }

            @Override public void forEachRemaining(Consumer<? super E> action) {
                requireNonNull(action); modCheckIterator.next(); iterator.forEachRemaining(element -> {
                    action.accept(element); modCheckIterator.next();
                }); }
        };
    }
