...
public class WrapQueue<V, Q extends java.util.Queue<V>> extends WrapCollection<V, Q> implements Queue<V> {
     ...
     */
    protected WrapQueue(Q wrapped) { super(wrapped); }
     ...
     *
     * @param wrapped the given {@link java.util.Queue} queue
     * @return wrapping {@link Queue} instance of the given {@link java.util.Queue} queue
     * @param <V> the type of elements
     * @throws NullPointerException if the given {@link java.util.Queue} queue is {@code null}
     */
    public static <V> Queue<V> wrap(java.util.Queue<V> wrapped) {
        return wrapped instanceof Deque ? WrapDeque.wrap(cast(wrapped))
                : wrapped instanceof BlockingQueue ? WrapBlockingQueue.wrap(cast(wrapped))
                : new WrapQueue<>(wrapped);
    }
}
