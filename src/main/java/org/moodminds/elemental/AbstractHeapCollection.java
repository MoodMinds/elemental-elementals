...
import org.moodminds.elemental.AbstractHeapCollection.Bucket.Link;
...
public abstract class AbstractHeapCollection<E, M extends Map<E, Object>>
     ...
     */
    protected transient final BiConsumer<Bucket<E>, E> bucketAccumulator = Bucket::put;
    ...
    protected Iterator<E> collectionIterator(Iterator<E> iterator) {
        return new Iterator<E>() {
            ...
            @Override public void forEachRemaining(Consumer<? super E> action) {
                requireNonNull(action); checkMod(expectedMod); iterator.forEachRemaining(action
                        .andThen(unused -> checkMod(expectedMod))); }
        };
    }
     ...
     */
    protected static class Bucket<E> extends AbstractLinkSequence<E, Link<E>> {
        ...
        protected void put(E element) {
            link(tail, new Link<>(element)); size++; }

        @Override
        protected void link(Link<E> previous, Link<E> next) {
            super.link(previous, tail = next);
        }

        @Override
        protected void unlink(Link<E> previous) {
            super.unlink(previous);
            if (previous == null) { if (head == null) tail = null;
            } else if (previous.next == null) tail = previous;
        }

        @Override
        protected SequenceIterator<E> iterator(Link<E> previous, Sequence<E> sequence, int index, Runnable removal) {
            return new SequenceIterator<E>() {

                AbstractLinkSequenceIterator<E, Link<E>> iterator;

                {
                    iterator = (AbstractLinkSequenceIterator<E, Link<E>>) Bucket.super.iterator(previous, sequence, index, () -> {
                        if (removal != null) removal.run(); unlink(iterator.previous); size--; iterator.index--;
                    });
                }
        ...
        protected class SubBucket extends AbstractLinkSubSequence {
            ...
            private transient final int bucketSize; private transient final Link<E> bucketTail;
            ...
            protected SubBucket(int bucketSize, Link<E> bucketTail, SubBucket parent, int fromIndex, int toIndex) {
            ...
            protected SequenceIterator<E> iterator(int index, Link<E> previous) {
                return new SequenceIterator<E>() {

                    final SequenceIterator<E> iterator = Bucket.super.iterator(previous, SubBucket.this, index, null);

                    @Override public boolean hasNext() {
                        return iterator.hasNext(); }
                    @Override public E next() {
                        checkMod(); return iterator.next(); }
                    @Override public int nextIndex() {
                        checkMod(); return iterator.nextIndex(); }
                    @Override public void remove() {
                        checkMod(); iterator.remove(); }
                    @Override public void forEachRemaining(Consumer<? super E> action) {
